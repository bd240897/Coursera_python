...
# ЧТО ДЕЛАЕТ - первоначально инициализирует
### - создает карту и все объекты на ней
### но кто вызывает это?
...
def restore_hp(engine, hero):
    ...
    engine.score += 0.1
    hero.hp = hero.max_hp
    engine.notify("HP restored") ### CОБЩЕНИЕ В ДАЛЬНЕЙШЕМ ПОЙДЕТ В ЧАСТ


def apply_blessing(engine, hero):
    """Применение эффекта Благославления или Берсерка к герою"""
    if hero.gold >= int(
        20 * 1.5**engine.level
...
def add_gold(engine, hero):
    """Побор золота и эффект слабости"""
...
class MapFactory(yaml.YAMLObject):
    """Фабрика классов - генератор карты"""
    @classmethod
    ...
    class Objects:
        ...
        def get_objects(self, _map):
                ...
                for obj_name in object_list_prob['enemies']:
                    prop = object_list_prob['enemies'][obj_name] ### характеристики персанажа
...
def service_init(sprite_size, full=True):
    ...
    # открыкаем потом чтоб считать информацию из файла ОБЪЕКТЫ , а потом закрываем - так работает это
    file = open("objects.yml", "r")

    object_list_tmp = yaml.load(file.read(), Loader=yaml.Loader) ### ЭТО загрузка ямл
    if full:
        object_list_prob = object_list_tmp   # ЛИСТО ПРОБ  - ЭТО ЗАГРУЗКУ ЯМЛ
    ...
    # цикл прохода по всем объектам и их отрисовка и добавление действий для них
    # stairs - лестница, chest - сундук
    # все описано в ямл файле objects.yml
    ...
    # открыкаем потом чтоб считать информацию из файла  УРОВНИ, а потом закрываем - так работает это
    if full:
