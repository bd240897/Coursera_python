...
# отрисовка всей информации - несколько классов - несоклько окон
...
class ProgressBar(ScreenHandle):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fill(colors["wooden"]) # цвет прогресс бара
...
class InfoWindow(ScreenHandle):
    ...
    def connect_engine(self, engine):
        ...
        """
        self.engine = engine
        engine.subscribe(self) # подписка на рссылку - если в игре что то произойдет, это выведется на экран
        if self.successor is not None:
            self.successor.connect_engine(engine)
