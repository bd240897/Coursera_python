...
def reload_game(engine, hero):
    ...
    engine.load_map(_map) # загрузить карту
...
class MapFactory(yaml.YAMLObject):
    ...
    def from_yaml(cls, loader, node):
        """Конструктор чтоб библа yaml поняла что делать с этим классом"""
        _map = cls.get_map()
...
def service_init(sprite_size, full=True):
    ...
    file.close()
    """Счиитываем данные из yml - отправляем туда открытый поток fopen - вернет нам словарь словарей"""
